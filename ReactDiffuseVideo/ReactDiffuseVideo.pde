...
void readNextFrame() {
  ...
  String fileName = "Skating/Skate" + num + ".png";
  startImg = loadImage(fileName);
...
void reactDiffuse() {
    image(startImg,0,0);
    filter(BLUR,2);
    PImage blurred = saveCurrent();
    
    image(blurred,0,0);
    filter(BLUR,4);
    PImage blurredHighPass = saveCurrent();
    
    PImage highPass = grainExtract(blurredHighPass, blurred);
    image(highPass,0,0);
    filter(BLUR, 2);
    highPass = saveCurrent();
